...
async function compile( json: any, syntaxTree: SyntaxTree, contentType: string ){
  ...
  for( let key in json ){
    ...
    if( typeof value == 'object' ){
      compiledJSON[ key ] = await compile( value, syntaxTree, getType( value ) )
      continue
    }
...
export default async ( json: any, syntaxTree: SyntaxTree, basePath?: string ) => {
  
  if( Object.entries( json ).length === 0 && json.constructor === Object )
    return {}
  
  if( basePath ) BASE_PATH = basePath
  return await compile( json, syntaxTree, getType( json ) )
}